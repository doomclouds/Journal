...
current draft 和 confirmed entry 只是参考材料，用于识别已有表达、重复内容和安全边界。
请摈弃当前草稿里已经形成但不合理的旧结构，重新规划整篇日记的九宫格分布。
请主动合并重复、移动错分、压缩冗余表达，并让每个 section 承担清晰主题。
不要保守地维持旧 section 分布；本按钮动作的目的就是重新组织结构。
...
3. Current draft / confirmed entry：参考材料，用于判断已有表达、重复内容、可修改范围和安全边界。
...
- 历史 raw inputs 是最高事实来源。
- current draft / confirmed entry 只作为参考，用于识别已有表达、重复内容和安全边界。
- 重新规划整篇日记的九宫格分布。
- 主动合并重复、移动错分、压缩冗余表达。
- 摈弃当前草稿里已经形成但不合理的旧结构。
- 不要保守地维持旧 section 分布。

即使是强结构重组，也不能删除、清空、覆盖或替换用户内容；必须通过允许的工具和 JMF validation 落地。
...
- current draft / confirmed entry 只作为参考。
- 检查重复、错分、冗余和九宫格边界。
- 可安全 revise 的 AI section 可以重写；用户触碰过的 section 只能 append。
