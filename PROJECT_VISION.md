...
> 版本：v1.0 / 0.1.0 release snapshot
> 日期：2026-05-15
...
截至 2026-05-15，主线已进入 V1 / `0.1.0` Windows 本地发布阶段：
...
自然语言输入
  ...
  -> 历史搜索 / 同日年轮 / 版本查看 / 今日版本恢复为草稿
  -> 数据导出 / 导入
  -> Windows 安装包
```

已经具备：今日输入、原始输入保存、Mock 兜底、OpenAI-compatible LLM 调用、Provider 配置、连接测试、受保护启用、API Key 掩码/查看、JMF 草稿预览、块编辑、源码编辑、结构校验、确认写入、Harness Core 受控工具调用、section 级 provenance、AI 审计工作台、覆盖前版本快照、可重建 SQLite/FTS5 历史索引、历史搜索工作台、只读同日年轮、今日版本恢复为待确认草稿、数据导出/导入、About/法律声明、安装版 Electron 托管 `.NET` backend、Inno Setup 安装包和 GitHub Actions release workflow。

仍未具备：非今日版本直接恢复/确认、跨日期编辑、AI 改写聊天、自动保存、应用内录音/语音转写、删除流程、item 级 provenance、draft diff、rollback、AI 多年总结、纪念日管理、Future Notes 完整体验、云同步、自动更新、代码签名和完整 API Key 导入导出。
...
当前同日年轮是只读记忆回廊：可以查看历史 Markdown、raw material snippets 和版本内容，但不在年轮模式里恢复版本；版本恢复仍属于普通历史工作台，并且当前只允许恢复今天的版本为 `reviewing` draft。
...
实现顺序说明：当前代码已进入 V1 / `0.1.0` Windows 本地发布阶段；路线图编号保留产品能力的逻辑分层，不代表严格完成顺序。
...
状态：Phase 6 已交付 Harness Core 与 AI 审计工作台；Phase 6B 已交付只读同日年轮。Future Notes 仍未交付为完整体验。
