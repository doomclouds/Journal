...
3. 渲染 Markdown 预览。
4. 等待用户确认或继续调整。
5. 用户确认后写入正式 Markdown、更新版本快照和 SQLite 检索索引。

这样可以避免模型直接生成 Markdown 时格式漂移，也避免 AI 在用户确认前污染正式日记。

### 4.4 今日整洁版

当天主 Markdown 文件始终表示用户确认过的当前最新版，适合用户阅读、编辑器打开和前端渲染。AI 预览不直接写入该文件；历史由版本目录保存。
...
- 一键确认保存。
- 继续和 AI 对话调整，例如“更自然一点”“保留更多原话”“把这条放到灵感”。
- 直接手动编辑 Markdown 预览。
- 暂存草稿，稍后再确认。
- 放弃本次 AI 草稿，但保留原始输入。
...
- 能力声明：声明是否支持 JSON mode、streaming、embedding 等。

第一版内置建议：

- `DeepSeek` 默认预设：普通用户只填写 API Key。
- `OpenAI` 默认预设：普通用户只填写 API Key。
- `智谱 GLM` 默认预设：普通用户只填写 API Key。
- `MockAiProvider`：本地假模型，用于开发、测试和无 Key 演示。
- `OpenAICompatibleProvider` 高级配置：面向专业用户，可自定义 baseUrl、model、协议兼容参数、超时、温度、JSON mode 等。

Ollama、本地模型、embedding、语音转写等作为后续 Provider 能力扩展。
...
public interface IAiProvider
{
    ...
    Task<AiModelListResult> ListModelsAsync(
        AiProviderConfig config,
        CancellationToken cancellationToken);

    Task<AiHealthCheckResult> CheckAsync(
        AiProviderConfig config,
        CancellationToken cancellationToken);
}
```
...
普通配置页只暴露 Provider 选择、API Key、连接测试和模型选择。高级配置页再暴露 baseUrl、model、超时、温度、JSON mode、streaming 等参数。API Key 不写入 Markdown，也不进入版本快照。具体凭据存储方式在实现阶段再按 Windows 桌面安全策略细化。
...
- 用户确认后，保存新 Markdown 前先将旧版写入 `versions`。
- 当前文件始终保持用户确认过的最新版。
...
- React：负责前端界面、Markdown 预览、状态呈现、配置页面；块编辑器和源码编辑器在 JMF 生成确认链路跑通后进入独立阶段。
- .NET 后端：负责本地 API、AI Provider、JMF v1 渲染与解析、文件存储、索引、版本管理。
- SQLite：作为本地索引库，不作为唯一数据源。
...
- Markdown：后端负责 JMF v1 渲染、解析、校验，前端先负责只读预览；块编辑和源码编辑在后续阶段实现
- 索引：SQLite + FTS5
...
API Key 不能进入 Markdown、版本快照、普通日志或错误报告。凭据存储需要单独设计。
...
- Markdown 使用 JMF v1；阶段 2 只做生成、预览和确认，普通用户默认块编辑模式从阶段 3 开始实现。
- 第一版不接入应用内语音。
- AI Provider 可插拔，第一阶段先保留接口和 Mock 实现。
