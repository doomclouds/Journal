...
截至 2026-05-14，主线已交付到 Phase 6B，并补上 Phase 4A 本地历史可靠性：
...
已经具备：今日输入、原始输入保存、Mock 兜底、OpenAI-compatible LLM 调用、Provider 配置、连接测试、受保护启用、API Key 掩码/查看、JMF 草稿预览、块编辑、源码编辑、结构校验、确认写入、Harness Core 受控工具调用、section 级 provenance、AI 审计工作台、覆盖前版本快照、可重建 SQLite/FTS5 历史索引、历史搜索工作台、只读同日年轮和今日版本恢复为待确认草稿。

仍未具备：非今日版本直接恢复/确认、跨日期编辑、AI 改写聊天、自动保存、应用内录音/语音转写、安装包、生产模式下 Electron 自动托管 .NET 后端、删除流程、item 级 provenance、draft diff、rollback、AI 多年总结、纪念日管理和 Future Notes 完整体验。
...
实现顺序说明：当前代码已交付 Phase 6B，并补上 Phase 4A 本地历史可靠性；路线图编号保留产品能力的逻辑分层，不代表严格完成顺序。
