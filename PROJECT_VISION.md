...
> 版本：v1.1 / main after 0.1.1 release
> 日期：2026-05-17
> 定位：基于《晨间日记的奇迹》思想的本地优先、AI 辅助、可检索可回看的 Markdown 桌面日记应用
...
日记必须有长期可读的稳定结构，但不能被固定模板锁死。默认固定“昨日回顾”和“今日重点”，其他内容由 AI 根据当天表达自动补充，例如状态、健康、关系、工作学习、财务、灵感和未来提醒等。
...
截至 2026-05-17，项目已经发布 `v0.1.0` 和 `v0.1.1` 两个 Windows 本地版本；主线在发布闭环之上继续推进同日记忆回廊和长期复盘体验：
...
自然语言输入
  ...
  -> 块编辑 / 源码编辑 / Harness 执行与 JMF 结构校验
  -> 用户确认
  -> 覆盖前版本快照
  -> 正式 Markdown 文件
  -> 可重建 SQLite/FTS5 历史索引
  -> 历史搜索 / 同日记忆回廊 / 版本查看 / 今日版本恢复为草稿
  -> 数据导出 / 导入
  -> Windows 安装包
```

已经具备：今日输入、原始输入保存、Mock 兜底、OpenAI-compatible LLM 调用、Provider 配置、连接测试、受保护启用、API Key 掩码/查看、JMF 草稿预览、块编辑、源码编辑、结构校验、确认写入、Harness Core 受控工具调用、section 级 provenance、AI 审计工作台、覆盖前版本快照、可重建 SQLite/FTS5 历史索引、历史搜索工作台、同日记忆回廊、纪念日资料、下一年提醒、今日版本恢复为待确认草稿、数据导出/导入、About/法律声明、安装版 Electron 托管 `.NET` backend、Inno Setup 安装包和 GitHub Actions release workflow。

仍未具备：非今日版本直接恢复/确认、跨日期编辑、AI 改写聊天、自动保存、应用内录音/语音转写、删除流程、item 级 provenance、draft diff、rollback、AI 多年总结、Future Notes 完整体验、云同步、自动更新、代码签名和完整 API Key 导入导出。
...
AI 不直接写入正式 Markdown。普通生成路径输出结构化 JSON；Harness 路径让模型只提交 side-effect-free 工具计划，再由服务端执行。后端负责：

1. 校验 JSON。
2. 归一化字段。
3. 渲染 Markdown 预览。
4. 等待用户确认或继续调整。
5. 在用户确认时先写覆盖前版本快照，再写正式 Markdown。
6. 同步更新可重建 SQLite/FTS5 历史索引。

这样可以避免模型直接生成 Markdown 时格式漂移，也避免 AI 在用户确认前污染正式日记。

### 4.4 今日整洁版

当天主 Markdown 文件始终表示用户确认过的当前最新版，适合用户阅读、编辑器打开和前端渲染。AI 草稿、Harness 执行、LLM 重生成和编辑保存都先落在 draft 边界内，不直接写入正式文件；正式文件被覆盖前会先写入 `.journal/versions/` 版本快照。

### 4.5 同日记忆回廊

围绕月日维度建立纵向关联。例如 5 月 7 日打开应用时，可以从日记纸面的“日记回廊”打开同日年轮，看到：

- 2026-05-07 的今日记录。
- 2025-05-07、2024-05-07 等历史同日记录。
- 同日记录中的 Markdown 正文、原始材料摘要和版本快照。

这会形成“人生年轮”体验：用户不是只看昨天，而是在看不同年份的自己。

当前同日记忆回廊已经从普通年份列表升级为三栏工作台：左侧做日期、常看纪念日和年份节点导航；中间展示多年同日时间轴卡片，并可切换到该日期最后正式日记阅读态；右侧管理纪念日意义、起点日期和写给下一年同一天的提醒。时间轴卡片只显示有限摘要，长文本会省略，最多显示 3 条预览，详细内容通过阅读态查看。

年轮模式仍然只读：不在年轮模式里恢复版本、删除日记、编辑正式 entry 或做 diff；版本恢复仍属于普通历史工作台，并且当前只允许恢复今天的版本为 `reviewing` draft。
...
用户自然语言文本
  -> 保存原始输入
  -> 调用 Mock / 真实 LLM Provider 或 Harness Planner
  -> AI 返回结构化 JSON 或受控工具计划
  -> 后端校验、执行并渲染 JMF Markdown 草稿
  -> 用户通过块编辑或源码编辑调整草稿
  -> 用户确认时先生成覆盖前版本快照
  -> 写入正式 Markdown
  -> 更新 SQLite / FTS 索引
...
当前 active 可选单例块，每篇最多一个；新内容、新增块菜单、今日编辑器可新增块列表、Harness prompt catalog 和 Harness operation executor 都只能面向这些 section：
...
legacy section 继续被 Parser、validator、composer 和显示层兼容，旧日记不会因为旧 section 失效；但它们不再作为新内容写入目标：

- `learning`：学习与思考，合并到 `work`。
- `future-notes`：未来提醒，合并到 `inspiration`。
- `gratitude`：感恩，合并到 `relationship`。

Harness executor 遇到 legacy target 时返回 `harness-target-inactive`，不把新内容写入 legacy section。

块顺序在第一版固定。active 可选块如果没有内容就不渲染；一旦出现，系统按固定顺序插入，而不是按 AI 输出顺序或用户拖拽顺序插入。历史 legacy 块保留解析和显示兼容，不做全量迁移。
...
- 块编辑模式：默认模式。用户只看到“原始输入、昨日回顾、今日重点、灵感与未来提醒”等块，不看到 `<!-- journal:section ... -->` 这类机器标记。用户只能编辑块内容，新增块时只能从尚未出现的 active 可选块类型中选择。
...
`entries` 是人类主目录，应该保持干净。`.journal` 是应用维护目录，用户通常不需要手工编辑。当前已包含版本快照、审计记录和可重建索引目录：
...
- 每次输入都追加到 `raw-inputs`。
- append-input Harness run 会持久化当前输入，但 Planner prompt 中当前输入只作为本次 user message，不混入历史 raw inputs context。
- reorganize-existing Harness run 不追加 raw input，只使用服务端固定 prompt、已有 raw inputs、section catalog 和工具约束，不读取当前 draft 或 confirmed entry。
- AI 生成 JSON 草稿或 Harness 工具计划后，由服务端校验、执行并渲染出 Markdown 预览，进入 `reviewing` 或 `attention` 状态。
- 用户确认前不覆盖 `entries` 下的正式 Markdown。
- 当前版本用户确认后才更新正式 Markdown；覆盖已有正式 entry 前必须先将旧版写入 `versions`。
- 写入正式 entry 后同步更新可重建 SQLite/FTS5 索引；索引失败不回滚正式 Markdown，但会进入 warning/attention 处理路径。
- 当前文件始终保持用户确认过的最新版。
...
- 日期详情、版本列表、版本内容查看。
- 今日版本恢复为 `reviewing` draft。
- 同日年轮按 `MM-DD` 查询多年同日记录。
...
- `entries`：日期、`month_day`、文件路径、标题、情绪、标签、状态、版本号、文件 hash。
...
- React：负责今日工作台、Markdown 预览、块编辑、源码编辑、状态呈现、LLM 配置页面、AI 审计工作台、历史工作台和同日年轮。
- .NET 后端：负责本地 API、AI Provider、Harness Core、JMF v1 渲染与解析、结构校验、文件存储、版本快照和历史索引。
- SQLite：作为本地历史索引库，不作为唯一数据源；Markdown、raw-input jsonl 和 version 文件才是源材料。
- Markdown 文件：作为用户日记可信源。

前端与后端通过本机 HTTP API 通信。开发阶段固定端口；安装版 Electron 会启动打包内置的 `.NET` 后端进程，并在窗口关闭时做生命周期管理。
...
- 索引：SQLite + FTS5，可从 Markdown、raw-input jsonl 和 version 文件重建
- 测试：xUnit / Vitest / Playwright 或 Electron 自动化测试
- 打包：Electron Builder 生成桌面产物，Inno Setup 生成 Windows x64 安装包
...
实现顺序说明：当前代码已完成 V1 Windows 本地发布闭环，并在 `0.1.1` 后继续推进记忆回廊体验；路线图编号保留产品能力的逻辑分层，不代表严格完成顺序。
...
状态：Phase 4A 已交付本地历史、搜索、版本快照、可重建 SQLite/FTS5 索引和今日版本恢复为草稿。
...
状态：Phase 6 已交付 Harness Core 与 AI 审计工作台；Phase 6B 已交付只读同日年轮；Phase 8 已将其升级为同日记忆回廊，包含时间轴卡片、正式日记阅读态、纪念日资料和下一年提醒。Future Notes 仍未交付为完整体验。

产物：

- Harness Core 受控 LLM 工具调用。
- AI 审计工作台。
- 历史同日记录。
- 同日年份卡片、历史 Markdown、原始材料摘要和版本查看。
- 同日记忆回廊时间轴、纪念日资料和下一年提醒。
- FutureNote 基础模型和今日应回看的未来提醒。
...
- 月度、年度、周年复盘。
- 未来提醒的完整生命周期管理。
- AI 辅助提炼阶段性成长、感恩、关系、能力和情绪稳定性变化。

### 阶段 7：长期可靠性

目标是让用户敢把长期人生记录交给这个应用。

状态：V1 已交付本地导入导出、安装版 Electron 托管 `.NET` backend、Inno Setup 安装包和 GitHub Actions release workflow；可选加密、自动更新、代码签名和云同步仍未交付。
