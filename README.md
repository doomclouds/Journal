...
- 新增块只允许从当前 active 可选单例块中选择，并按固定顺序插入。
- `learning`、`future-notes`、`gratitude` 等 legacy section 仍可解析和显示，旧日记不因此 invalid，但不作为新内容或新增块目标。
...
- Harness Planner 的 section catalog 只暴露 active sections，并携带主题语义和避让规则；同一事实应进入一个最合适的 section，具体工作、学习、健康等主题不应重复堆进 `today-focus`。
- `learning`、`future-notes`、`gratitude` 等 legacy section 继续兼容解析和显示；Harness executor 不允许把新内容写入 legacy target，命中时返回 `harness-target-inactive`。
- 今日页按钮“重新整理”是用户选择式单篇转换路径：以历史 raw inputs 为唯一日记事实来源，放弃现有全部日记正文，不让 LLM 参考或继承当前 draft / confirmed entry，并按当前 active 分类重新整理草稿；用户确认后才覆盖正式 entry。
