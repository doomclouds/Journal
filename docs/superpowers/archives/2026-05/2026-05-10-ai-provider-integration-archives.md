...
阶段 5 把 Journal 从纯 Mock 整理推进到真实可配置 LLM Provider：后端保留 `AI Provider` 技术抽象和 JMF 校验边界，真实模型统一走 OpenAI-compatible runtime，前端提供 `LLM` 状态入口和配置面板；未配置时仍默认 Mock，真实 Provider 失败时进入 attention，不静默回退、不写正式 entry。

## Delivered Scope

- 新增异步 AI Provider 合同、动态 provider/model/prompt metadata、settings store、环境变量覆盖、默认 Provider 预设，以及 OpenAI-compatible Agent Framework runtime。
- `TodayJournalService` 改为通过 generation service 选择 active Provider，AI 输出仍只生成 `JournalAiJson`，并经过 validator 和 JMF renderer 后进入 reviewing/attention draft。
...
- `dotnet test Journal.slnx`：119/119 .NET tests passed。
- `npm test --prefix apps/desktop`：48/48 frontend tests passed。
...
- `测试已保存配置` 明确只验证已保存 Provider，不验证当前未保存表单草稿，避免 UI 语义误导。
