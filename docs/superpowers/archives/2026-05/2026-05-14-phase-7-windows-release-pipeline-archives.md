...
最终验收修复补齐了 packaged Electron 的发布阻塞点：Vite 产物使用相对 `./assets/...` 路径，`file://` renderer 的 `Origin: null` 预检被后端精确允许，实际请求必须携带本次 Electron 启动分配的 desktop access token。Electron 主窗口禁止外链导航接管 renderer，packaged backend 启动时强制 `ASPNETCORE_ENVIRONMENT=Production`，同时 release metadata 将同一个 tag/release version 注入 frontend/backend，避免 About 面板和导出 manifest 显示漂移版本。

## Delivered Scope

- About 面板展示 release、frontend、backend、commit、build time 和数据目录；当前产品形态下 frontend/backend/release 使用同一个 tag version。
...
- Final packaged smoke 固化在脚本与测试里：`stage-installer.ps1` 检查 build metadata 必含 release、frontend、backend version，并拒绝 root-relative `/assets/...` 产物。
...
- Release staging now treats frontend asset path shape and release metadata as build-time gates, so future installer builds fail before shipping a white-screen package or stale About version.
...
- 2026-05-16 v0.1.1 回访：`/app/info` 新增 `backendVersion`，release scripts 将同一个 tag/release version 写入 `JOURNAL_RELEASE_VERSION`、`JOURNAL_FRONTEND_VERSION` 和 `JOURNAL_BACKEND_VERSION`；Electron 复用/重启 packaged backend 时按 backend version 判定 stale，避免 About 中 Backend 继续显示旧静态版本。
