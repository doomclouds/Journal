...
同日年轮把“今天的月日”变成历史回看的入口：用户可以从日记纸面的回廊菜单打开历史工作台的 anniversary mode，按 `MM-DD` 回看历年同一天的正式日记、原始材料摘要和版本快照，而不改变当前今日日记的主视觉与编辑流程。
...
- 桌面端新增独立三栏 `AnniversaryWheelWorkbench`，包含日期选择、年份卡片、Markdown 预览、原始材料摘要和版本查看。
- 日记纸面新增 `日记回廊` 菜单入口，支持顶部按钮和正文右键菜单；`同日年轮` 默认使用今天的 `monthDay`，并补齐 detail/version 异步请求 stale guard。
- 前端 month-day 输入与后端语义对齐：`02-29` 合法，非法日期不会发起 anniversary 请求，也不会把旧结果挂到新标题下。

## Out of Scope

- 同日年轮是只读记忆回廊，不提供任何版本恢复入口；即使选中今天的同日记录，也只能查看版本。
...
- 本次年轮复用了 History Workbench 的版本查看能力，但不复用恢复动作；恢复仍只属于普通 history mode 的编辑恢复链路。
