...
后续同日年轮切换日期时又暴露一个更轻量但同源的症状：anniversary summary 已返回、entry detail 尚未返回的中间帧里，中间日记区会先渲染多条 `hits` 摘要块，看起来像编辑界面的引用块，随后才切成 Markdown 日记正文。
...
- `anniversaryResult.items` 只包含 summary/hits，不是完整 Markdown entry；切换日期时它可能早于 `getJournalHistoryEntry()` 返回。
...
同日年轮摘要闪烁的根因是组件把 `selected.hits` 当成“没有 Markdown 时的通用 fallback”。但对 `processed`、`updated`、`reviewing`、`attention` 这些本应有详情的日期来说，`matchingDetail === null` 往往只是 detail 请求还在路上，而不是“这一天没有正文”。summary/hits 只能作为缺失日期或材料回顾使用，不能临时占据日记正文区域。
...
- `AnniversaryWheelWorkbench` 区分“等待完整 entry detail”和“确实没有正文可展示”：非 `missing` 日期在 `matchingDetail === null` 时显示读取中状态，不渲染 `selected.hits`；组件测试覆盖 summary 已有但 detail 未到时不闪出摘要块。
...
对同日年轮正文区，选择显式 loading 而不是继续显示 summary hits，是因为“记忆回廊”的主阅读区应只展示完整日记或版本内容；summary hits 留在年份列表和右侧材料区更合理，也避免了短暂 UI 形态跳变。
...
- 切换同日年轮日期时，中间主阅读区先出现多条 section/raw-input 摘要块，随后才变成 Markdown 日记正文。
