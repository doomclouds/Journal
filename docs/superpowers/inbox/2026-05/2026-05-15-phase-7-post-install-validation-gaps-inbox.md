...
- 是否需要把 Electron preload bridge 检查、About 声明入口和本地路径打开能力加入 release verification checklist。
- 如果后续安装包仍复现，需要另行确认是旧包未更新、安装版启动链路、权限或系统 shell 行为，而不是重复归因到 preload sandbox。

## Likely Next Route

Electron preload bridge 失效部分已提升为正式 problem。剩余 About/安装包 smoke 与 release checklist 覆盖缺口仍可留在 inbox；如果重新打包后的安装版仍暴露本地路径能力问题，再创建或更新更具体的问题资产。
