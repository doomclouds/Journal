...
> 日期：2026-05-10
> 状态：已实现并归档
> 对应愿景：`PROJECT_VISION.md` 阶段 5：真实 LLM 接入
...
本阶段采用 **OpenAI-compatible Runtime + Microsoft Agent Framework 1.5.0 轻量调用**。LLM 配置支持 `Mock`、`OpenAI`、`DeepSeek`、`智谱 GLM`、`Custom OpenAI-Compatible`。真实 LLM 走统一 OpenAI-compatible 调用路径；Mock 继续作为未配置时的默认能力和测试能力。
...
- 通过顶部 `LLM` 状态入口查看当前 LLM。
- 配置并启用 `OpenAI`、`DeepSeek`、`智谱 GLM` 或自定义 OpenAI-compatible LLM。
- 使用环境变量覆盖本机配置。
- 手填模型名，不依赖 LLM 一定支持 `/models`。
- 点击“测试已保存配置”发送最小 JSON 请求验证已保存的 base URL、API Key、model 和 JSON 输出能力。
- 点击“重新整理草稿”用当前 LLM 重新生成当天 draft。
- 在真实 LLM 失败时看到 `attention` 状态、友好摘要和可展开安全技术详情。
...
所有真实 LLM 统一走 OpenAI-compatible chat completion 调用。`OpenAI`、`DeepSeek`、`智谱 GLM` 是预设配置，`Custom OpenAI-Compatible` 是用户自定义配置。这样可以避免一开始为每家模型服务写重复 adapter，也方便后续接入 Ollama、OpenRouter、硅基流动或本地代理。
...
不把“生成、校验、修复、渲染”交给 Agent 自主规划。这个流程是确定业务流，不需要模型或 workflow 接管。Agent Framework 在首版只是 LLM runtime 的技术底座。

### 4.3 Mock 是未配置默认，不是真实失败的静默兜底

未配置真实 LLM 时，系统默认使用 `MockAiProvider`，保证本地演示、自动测试和无 Key 使用不被卡住。

一旦用户启用真实 LLM，真实调用失败必须进入 `attention`，不能静默回退 Mock。静默回退会让用户误以为真实模型生成成功，破坏长期日记的来源可信度。
...
配置文件存完整 LLM 列表，包括预设配置。这样用户可以修改默认模型名、display name、启用状态和自定义 base URL，文件结构也更直观。
...
连接测试发送一次最小 JSON 请求。首版测试的是**已保存 LLM 配置**，不是当前表单里尚未保存的草稿；用户修改 model、base URL 或 API Key 后，需要先保存再测试。
...
- API Key 可用。
- model 存在。
- LLM 支持 JSON object 输出。
- 返回能解析为 JSON。

UI 必须提示：“测试会向当前 LLM 发送一次最小请求，可能产生少量 token 消耗。”
