...
Journal 已发布 `v0.1.0` 和 `v0.1.1` 两个 Windows 本地版本；主线已进入 Phase 8 同日记忆回廊后的体验增强阶段。根 `AGENTS.md` 只保留 agent 启动入口、硬规则和资产复利导航；长项目上下文已拆到引用文档里。
...
Current main includes local-first daily writing, raw input preservation, JMF draft/edit/validation/confirmation, OpenAI-compatible LLM settings, Harness Core audit runs, rebuildable local history index, history search, same-day memory corridor, saved anniversaries, next-year notes, data backup/import, Windows installer scripts, and GitHub Actions release workflow.

Same-Day Memory Corridor is no longer a simple year list: the accepted UI contract is a three-column workbench where the left rail provides date / saved anniversary / year-node navigation, the middle shows timeline cards and formal diary reading mode, and the right panel manages anniversary meaning and next-year notes. Timeline cards are summaries only; long preview text is truncated and only the first three preview lines are shown.
...
Use the intended tag/release version for new releases, for example `0.1.1`; do not assume the examples above are the current target version.
